...
def from_dataset(dataset: Iterator[Tuple[str, Rect]], sizes: List[int], steps: List[int]) -> Iterator[
    Tuple[float, ndarray]]:
    dataset = map(lambda x: (image.from_path(x[0]), x[1]), dataset)

    return chain.from_iterable(
        map(lambda x: from_array(x[0], x[1], steps=list(map(_to_tuple, steps)), sizes=list(map(_to_tuple, sizes))),
            dataset)
    )
